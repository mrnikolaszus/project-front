...
function show_list(page_number){
    ...
    let pageCount = $("#pages_selector").val();
    if(pageCount == null){
        pageCount=5;
    }

    url = url.concat("pageSize=").concat(pageCount);
    ...
    $.get(url, function (response){
        $.each(response, function (i, item) {
            $('<tr>').html("<td>"
                ...
                + item.banned + "</td><td>"
                + "<button class='but' id='button_edit_" + item.id + "' onclick='editAcc(" + item.id + ")'>"
                + "<img src='/img/edit.png'"
                + "</button>" + "</td><td>"
                + "<button class='but' id='button_delete_" + item.id + "' onclick='deleteAcc(" + item.id + ")'>"
    ...
    let count = getFromServer();

    let pageCountFact = Math.ceil(count/pageCount);

    $("button.pgn-btn-styled").remove();

    for (let i = 0; i <pageCountFact; i++){
...
function editAcc(id){
    ...
    td_race.innerHTML = getRaceHtml(id);
    ...
    td_profession.innerHTML = getProfHtml(id);
    ...
    td_banned.innerHTML = getBannHtml(id);
...
function createAcc(){
    ...
    let value_profession = $("#input_prof_new").val();
